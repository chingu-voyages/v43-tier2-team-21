

const Hero = () => {
    return (
      <section className="flex items-center flex-col text-white bg-purple-600 py-40">
        <img src="./images/skills.png" alt="skills img" className="w-40 text-white" />
        <h1 className="text-6xl pt-20 font-bold">Skill Tracker</h1>
        <h2 className="text-2xl">Start Learning Something New Today!</h2>
        <button
          className="justify-center cursor-pointer px-5 py-2 my-10 rounded-3xl text-lg bg-white text-purple-600 hover:bg-gray-500 hover:text-black"
          onClick={() => console.log('button clicked')} // Connect to sign-in modal
        >
          Sign Up Now!
        </button>
      </section>
    );
}

export default Hero;